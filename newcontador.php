...
if ( $array2->success==false){
    ...
    $consulta="UPDATE `masgps`.`hash` SET `hash` = '$hashed' WHERE (`id` = '16');";
...
foreach ($ids as $id) {
    ...
    $consulta="SELECT MAX(entrada) as maximo FROM `masgps`.`contadorinter`  WHERE patente= '$patente2' AND fechaConsulta='$hoy'" ;

    $consultasalida="SELECT MAX(salida) as maximoS FROM `masgps`.`contadorinter`  WHERE patente= '$patente2' AND fechaConsulta='$hoy'" ;
    ...
    $query = "INSERT INTO `masgps`.`contadorinter` (trakerId,latitud,longitud,direccion,fecha,patente,entrada,entradaT,salida,salidaT,aforo,fechaConsulta,fechafiltro) 
